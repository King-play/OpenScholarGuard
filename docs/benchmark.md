...
`scholarguardbench-v0` is the formal v0 seed dataset. It now contains 36 synthetic
task-style cases: 5 clean controls and 31 attack or integrity-risk cases across these
families:
...
modality, source, family, expected detectors, expected severity, task ID, split,
difficulty, verifier, and expected action.

See [docs/dataset-card.md](dataset-card.md) for the dataset card, known limitations,
evaluation contract, and expansion plan.
...
ground-truth labels, expected detectors, expected severity, file paths, and formal
task metadata. The structure follows the same principle as mature skill/task benchmarks:
each sample is a small task with a stable verifier and an expected action, rather than
just an unstructured prompt.
...
Each `scholarguardbench-v0` manifest sample carries:

- `task_id`: stable task identifier, currently `sgb-v0-<case-id>`.
- `split`: current public split, `dev`.
- `difficulty`: `easy`, `medium`, or `hard`.
- `verifier`: deterministic verifier contract, currently `detector-match`.
- `expected_action`: `allow` for clean controls, `flag` for attacks or integrity risks.
- `target_workflow`: `ai-review`, `rag-ingestion`, `vlm-review`, `document-agent`, or `submission-screening`.
- `visibility`: whether the payload is human-visible, hidden rendered text, metadata, OCR layer, image text, or retrieved context.
- `modality`: text, HTML, LaTeX, OCR text, image text, PDF metadata, or related document modality.
